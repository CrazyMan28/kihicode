...
import runPythonAgent from '../utils/runPythonAgent';
...
export const App: React.FC<{ initialDir: string }> = ({ initialDir }) => {
  const [query, setQuery] = useState('');
  const [messages, setMessages] = useState<Message[]>([]);
  const [status, setStatus] = useState<string>('idle');
  const [currentTool, setCurrentTool] = useState<string | null>(null);
  const [permissionRequest, setPermissionRequest] = useState<{ id: string; tool: string; args: any } | null>(null);
  const [showHelp, setShowHelp] = useState(false);
  const [theme, setTheme] = useState<'dark' | 'light'>('dark');
  const [scrollOffset, setScrollOffset] = useState(0);
  const [autoScroll, setAutoScroll] = useState(true);
  const [clock, setClock] = useState(() => new Date().toLocaleTimeString());
  const [modeState, setModeState] = useState<'safe' | 'yolo'>((process.env.KIHICODE_MODE as 'safe' | 'yolo') || 'safe');
  const { exit } = useApp();
  ...
  useEffect(() => {
    ...
    const envMode = process.env.KIHICODE_MODE as unknown as 'safe' | 'yolo' | undefined;
    if (envMode) {
      setTimeout(() => {
        // @ts-ignore
        if (agent && typeof agent.setMode === 'function') agent.setMode(envMode);
      }, 0);
    }
      ...
      try {
        ...
        for (const e of entries) {
          try {
            ...
            if (stat.isFile()) fileNames.push(e);
          } catch {
            // ignore
          }
          if (fileNames.length >= 9) break;
        }
        setFiles(fileNames);
      } catch {
        setFiles([]);
  ...
  const framedBoxLines = (width: number, height: number, contentLines: string[]) => {
    ...
    for (let i = 0; i < bottomPad; i++) lines.push(empty);
    lines.push(bottom);
    return lines;
  };

  const flattenMessagesToLines = (msgs: Message[]) => {
    const out: { text: string; role?: Message['role'] }[] = [];
    for (const m of msgs) {
      out.push({ text: `${m.role.toUpperCase()}:`, role: m.role });
      const parts = m.content.split('\n');
      for (const p of parts) out.push({ text: `  ${p}`, role: m.role });
      out.push({ text: '' });
    }
    return out;
  };
  ...
  const handleSubmit = async (value: string) => {
    ...
    if (value.startsWith('/mode')) {
      const newMode = value.split(' ')[1] as 'safe' | 'yolo';
    ...
    if (value.startsWith('/')) {
      ...
      const cleaned = tokens.map((t: string) => t.replace(/^"(.*)"$/, '$1'));
      const nameToken = cleaned[0] || '';
      const name = nameToken.startsWith('/') ? nameToken.slice(1) : nameToken;
      const cmd = registry.get(name);
      if (cmd) {
        const ctx = {
          stdout: (msg: string) => setMessages(prev => [...prev, { role: 'system', content: msg }]),
          exit: (code?: number) => setMessages(prev => [...prev, { role: 'system', content: `Exit(${code ?? 0})` }]),
          registry,
          subagentManager,
          authStore,
        } as any;
      ...
      try {
        ...
        if (res.stdout) setMessages(prev => [...prev, { role: 'assistant', content: res.stdout }]);
        if (res.stderr) setMessages(prev => [...prev, { role: 'system', content: `Agent stderr: ${res.stderr}` }]);
      } catch (err: any) {
    ...
    try {
      await agent.run(value, (update: any) => {
  ...
  return (
    <Box flexDirection="column" padding={1}>
      ...
      {paletteOpen && (
        <Box marginBottom={1}>
          <Palette
            commands={registry.list().map((c: any) => ({ name: c.name, description: c.description }))}
            onSelect={(name: string) => { setQuery('/' + name + ' '); setPaletteOpen(false); }}
            onClose={() => setPaletteOpen(false)}
          />
        </Box>
      )}
      {/* Top dashboard (responsive) */}
      <Box flexDirection="row" gap={1} marginBottom={1}>
        <Box flexGrow={3} paddingRight={1}>
          <Text color={themeColors.header} bold>Kihicode v1.0.0</Text>
          <Text color={themeColors.muted}>--- Agentic AI CLI ---</Text>
          <Box marginTop={1} />
          ...
          {(() => {
            ...
            return <Text color={themeColors.muted}>{leftLines.join('\n')}</Text>;
          })()}
          {/* quick files bar (numbered 1-9 to open) */}
          <Box marginTop={1}>
            <Text color={themeColors.accent}>Files:</Text>
            <Text> {files.map((f, i) => `[${i+1}] ${f}`).join('  ')}</Text>
          </Box>
        </Box>
          ...
          {(() => {
            ...
            return (
              <Box flexDirection="column">
                <Text color={themeColors.muted}>{recentLines.join('\n')}</Text>
                <Box marginTop={1} />
                <Text color={themeColors.muted}>{freshLines.join('\n')}</Text>
              </Box>
            );
          })()}
        </Box>
      ...
      {showHelp && (
        <Box borderStyle="double" padding={1} flexDirection="column" marginBottom={1}>
          <Text bold color={themeColors.accent}>Keyboard Shortcuts</Text>
          <Box marginTop={1} />
          <Text color={themeColors.muted}>Ctrl+H — Toggle this help</Text>
          <Text color={themeColors.muted}>Ctrl+Q — Quit</Text>
          <Text color={themeColors.muted}>Ctrl+T — Toggle theme</Text>
          <Text color={themeColors.muted}>Ctrl+B — Background current command (simulated)</Text>
          <Text color={themeColors.muted}>Enter — Submit input</Text>
          <Box marginTop={1} />
          <Text color={themeColors.muted}>Type /help for commands. Press Ctrl+H to close.</Text>
        </Box>
      )}

      {/* Messages area */}
      <Box borderStyle="single" padding={1} flexDirection="column" flexGrow={1} marginBottom={1}>
        {/* Render only the visible slice of message lines to prevent terminal scroll and allow internal scrolling */}
        {(() => {
          ...
          if (visible.length === 0) return <Text color={themeColors.muted}>No messages yet — try a command or chat with the assistant.</Text>;
          ...
          if (totalLines > trackHeight) {
            ...
            for (let j = 0; j < trackHeight; j++) scrollbarChars.push(' ');
          }
              ...
              {visible.map((ln, i) => {
                ...
                if (!ln.text) return (
                  <Box key={i} flexDirection="row">
                    ...
                    <Box width={2}><Text color={themeColors.accent}>{scChar}</Text></Box>
                  </Box>
                );
                ...
                return (
                  <Box key={i} flexDirection="row">
                    <Box flexGrow={1}>
                      {isLabel ? (
                        <Text bold color={color}>{ln.text}</Text>
                      ) : (
                        <Text>{ln.text}</Text>
                      )}
                    </Box>
                    <Box width={2}>
                      <Text color={scChar === '█' ? themeColors.accent : themeColors.muted}>{scChar}</Text>
                    </Box>
                  </Box>
                );
              })}
      ...
      {/* Divider */}
      <Box>
        <Text color="gray">{Array(Math.max(10, (process.stdout?.columns || 80) - 2)).fill('─').join('')}</Text>
      </Box>
      ...
      <Box>
        ...
        <Box flexDirection="column" flexGrow={1}>
          <TextInput value={query} onChange={setQuery} onSubmit={handleSubmit} placeholder={'try "edit < filepath > to ..."'} />
          {status !== 'idle' && (
            <Box marginTop={1}>
              <Text color="yellow"><Spinner type="dots" /> {status} {currentTool && `(${currentTool})`}</Text>
            </Box>
          )}
        </Box>
      </Box>
      ...
      <Box marginTop={1} paddingX={1}>
        <Box flexGrow={1}>
          <Text color={themeColors.muted}>{clock}</Text>
        </Box>
        <Box>
          <Text color={themeColors.accent}>Mode: {modeState}</Text>
        </Box>
        <Box marginLeft={2}>
          <Text color="gray">{initialDir}</Text>
        </Box>
      </Box>
    </Box>
  );
