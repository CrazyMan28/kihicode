...
export async function validateProviderKey(provider: string, apiKey: string): Promise<boolean> {
  ...
  try {
    if (provider === 'openai') return await openaiValidate(apiKey);
    if (provider === 'anthropic') return await anthropicValidate(apiKey);
    if (provider === 'huggingface') return await hfValidate(apiKey);
  } catch (err) {
    return false;
  }
  // fallback heuristic
